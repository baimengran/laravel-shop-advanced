    ...
    public function handle()
    {
        ...
        foreach ($installment->items as $item) {
            ...
            try {
                ...
                //如果某一个还款计划报错，则暂时跳过，继续下一个还款计划的退款
                continue;
            }
        }
        //判断分期还款计划的退款师傅全部成功
        ...
        switch ($item->payment_method) {
            case 'wechat':
                app('wechat_pay')->refund([
                    'transaction_id' => $item->payment_no,//使用微信订单号退款
                    'total_fee' => $item->total * 100,//原订单金额，单位分
                    'refund_fee' => $item->base * 100,//要退款的订单金额，单位分，分期退款只退本金
                    'out_refund_no' => $refundNo,//退款订单号
                    //微信支付的退款结果不是事实返回，而是通过退款回调通知，因此，这里需配置退款回调接口地址
                    'notify_url' => ngrok_url('\'installments.wechat.refund_notify\'')//微信退款回调,
                ]);
                //将还款计划退款状态改成退款中
                $item->update([
            ...
            case 'alipay':
                $ret = app('alipay')->refund([
                    'trade_no' => $item->payment_no,//使用支付宝订单号退款
                    'refund_amount' => $item->base,//退款金额，单位元，只退本金
