@extends('layouts.app')
@section('title',($address->id?'修改':'新增').'新增收获地址')
        ...
        <div class="col-lg-10 col-lg-offset-1">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @endif
                    {{--提交后端报错结束--}}
                    {{--inline-template内联方式引入change事件监测组件--}}
                    <user-addresses-create-and-edit inline-template>
                        @if($address->id)
                            <form class="form-horizontal" role="form" action="{{route('user_addresses.update',['user_address'=>$address->id])}}"
                                  method="post">
                                {{method_field('put')}}
                                @else
                                    ...
                                    {{--引入csrf token字段--}}
                                    {{csrf_field()}}
                                    <!-- inline-template 代表通过内联方式引入组件 并添加change事件监测 -->
                                        {{--:init-value :表示vue中v-bind的缩写，指明后面的值是一个表达式而不是字符串--}}
                                        <select-district
